...
def convert_xls_to_xlsx(file_path, output_dir):
    ...
    xlsx_file_path = f"{output_dir}/converted.xlsx"
...
def process_file(file_path, output_dir):
    try:
        ...
        if file_path.endswith('.xls'):
            file_path = convert_xls_to_xlsx(file_path, output_dir)
        ...
        for _, row in sales_data_df.iterrows():
            date = pd.to_datetime(row['Order Date'], errors='coerce')
            if pd.isna(date):
                continue
            month = date.month
            purchases = row['Purchase']
            
            # Ensure purchases are treated as strings
            if not isinstance(purchases, str):
                continue

            items = purchases.split()
            
            # Ensure the items list has pairs of quantity and product keys
            if len(items) % 2 != 0:
                continue
            ...
            for i in range(0, len(items), 2):
                try:
                    ...
                    # Handle cases where the conversion to int fails or list index is out of range
                    continue
        ...
        output_file_path = f"{output_dir}/sales_data_with_monthly_sales.xlsx"
        with pd.ExcelWriter(output_file_path, engine='openpyxl', mode='w') as writer:
            output_df.to_excel(writer, sheet_name='Monthly Sales', index=False)

        messagebox.showinfo("Success", f"Monthly sales data has been successfully written to '{output_file_path}'.")
...
def select_output_dir():
    ...
    if output_dir:
        output_dir_label.config(text=f"Selected output directory: {output_dir}")
        return output_dir
    return None

def go_button_clicked():
    file_path = file_label.cget("text").replace("Selected file: ", "")
    output_dir = output_dir_label.cget("text").replace("Selected output directory: ", "")
...
root.geometry("600x300")  # Set the default size
...
# Create and place the "Select Output Directory" button
